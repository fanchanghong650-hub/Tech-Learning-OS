...
class Settings:
    ...
    def notes_path(self) -> str:
        slug = self.current_book.lower().replace(" ", "-") if self.current_book else "general"
        chapter = self.current_chapter.lower().replace(" ", "-") if self.current_chapter else "misc"
        subdir = "Reading" if self.mode == "tech" else "LN"
        return os.path.join(self.notes_dir, subdir, slug, chapter)
